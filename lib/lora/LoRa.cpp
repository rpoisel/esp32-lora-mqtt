...
LoRaClass::LoRaClass()
    : _spiSettings(8E6, MSBFIRST, SPI_MODE0), _ss(LORA_DEFAULT_SS_PIN),
      _reset(LORA_DEFAULT_RESET_PIN), _dio0(LORA_DEFAULT_DIO0_PIN), _frequency(0), _packetIndex(0),
      _implicitHeaderMode(0), _onReceive(NULL)
{
...
void LoRaClass::setTxPower(int8_t power, int8_t outputPin)
{
  ...
  if ((paConfig & RF_PACONFIG_PASELECT_PABOOST) == RF_PACONFIG_PASELECT_PABOOST)
  {
    ...
    if ((paDac & RF_PADAC_20DBM_ON) == RF_PADAC_20DBM_ON)
    {
      ...
      paConfig =
          (paConfig & RF_PACONFIG_OUTPUTPOWER_MASK) | (uint8_t)((uint16_t)(power - 5) & 0x0F);
    }
    else
    {
      if (power < 2)
      ...
      paConfig =
          (paConfig & RF_PACONFIG_OUTPUTPOWER_MASK) | (uint8_t)((uint16_t)(power - 2) & 0x0F);
    }
  }
  else
  {
...
void LoRaClass::handleDio0Rise()
{
  ...
  if ((irqFlags & IRQ_PAYLOAD_CRC_ERROR_MASK) == 0)
  {
    ...
    // read packet length
    int packetLength =
        _implicitHeaderMode ? readRegister(REG_PAYLOAD_LENGTH) : readRegister(REG_RX_NB_BYTES);
    // set FIFO address to current RX address
    writeRegister(REG_FIFO_ADDR_PTR, readRegister(REG_FIFO_RX_CURRENT_ADDR));
    if (_onReceive)
    {
